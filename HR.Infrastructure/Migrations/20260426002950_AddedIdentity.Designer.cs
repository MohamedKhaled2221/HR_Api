// <auto-generated />
using System;
using HR.Infrastructure.Persistence;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

#nullable disable

namespace HR.Infrastructure.Migrations
{
    [DbContext(typeof(HrDbContext))]
    [Migration("20260426002950_AddedIdentity")]
    partial class AddedIdentity
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "8.0.25")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder);

            modelBuilder.Entity("HR.Domain.Entities.Attendance", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<TimeSpan?>("CheckInTime")
                        .HasColumnType("time");

                    b.Property<TimeSpan?>("CheckOutTime")
                        .HasColumnType("time");

                    b.Property<DateTime>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime2")
                        .HasDefaultValueSql("GETUTCDATE()");

                    b.Property<DateTime>("Date")
                        .HasColumnType("datetime2");

                    b.Property<decimal?>("DeductionHours")
                        .HasColumnType("decimal(18,2)");

                    b.Property<int>("EmployeeId")
                        .HasColumnType("int");

                    b.Property<bool>("IsActive")
                        .HasColumnType("bit");

                    b.Property<decimal?>("OvertimeHours")
                        .HasColumnType("decimal(18,2)");

                    b.Property<int>("Status")
                        .HasColumnType("int");

                    b.Property<DateTime>("UpdatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime2")
                        .HasDefaultValueSql("GETUTCDATE()");

                    b.HasKey("Id");

                    b.HasIndex("EmployeeId", "Date")
                        .IsUnique()
                        .HasFilter("[IsActive] = 1");

                    b.ToTable("Attendances");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 1,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 2,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 2, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 1,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 3,
                            CheckInTime = new TimeSpan(0, 10, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 3, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 1m,
                            EmployeeId = 1,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 4,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 4, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 1,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 5,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 5, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 1,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 6,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 6, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 1,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 7,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 7, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 1,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 8,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 1,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 9,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 9, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 1,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 10,
                            CheckInTime = new TimeSpan(0, 10, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 10, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 1m,
                            EmployeeId = 1,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 11,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 11, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 1,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 12,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 12, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 1,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 13,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 13, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 1,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 14,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 14, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 1,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 15,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 15, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 1,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 16,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 16, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 1,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 17,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 19, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 17, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 1,
                            IsActive = true,
                            OvertimeHours = 2m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 18,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 18, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 1,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 19,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 19, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 1,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 20,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 20, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 1,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 21,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 21, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 1,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 22,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 22, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 1,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 23,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 23, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 1,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 24,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 24, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 1,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 25,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 25, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 1,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 26,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 26, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 1,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 27,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 27, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 1,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 28,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 28, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 1,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 29,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 29, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 1,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 30,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 30, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 1,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 31,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 2,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 32,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 2, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 2,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 33,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 3, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 2,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 34,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 4, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 2,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 35,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 5, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 2,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 36,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 6, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 2,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 37,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 7, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 2,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 38,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 2,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 39,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 9, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 2,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 40,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 10, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 2,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 41,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 11, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 2,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 42,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 12, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 2,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 43,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 13, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 2,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 44,
                            CheckInTime = new TimeSpan(0, 10, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 14, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 1m,
                            EmployeeId = 2,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 45,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 15, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 2,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 46,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 16, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 2,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 47,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 17, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 2,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 48,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 18, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 2,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 49,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 19, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 2,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 50,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 20, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 2,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 51,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 21, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 2,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 52,
                            CheckInTime = new TimeSpan(0, 10, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 22, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 1m,
                            EmployeeId = 2,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 53,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 23, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 2,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 54,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 24, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 2,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 55,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 25, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 2,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 56,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 26, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 2,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 57,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 27, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 2,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 58,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 28, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 2,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 59,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 29, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 2,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 60,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 30, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 2,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 61,
                            CheckInTime = new TimeSpan(0, 8, 30, 0, 0),
                            CheckOutTime = new TimeSpan(0, 16, 30, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 3,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 62,
                            CheckInTime = new TimeSpan(0, 8, 30, 0, 0),
                            CheckOutTime = new TimeSpan(0, 18, 30, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 2, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 3,
                            IsActive = true,
                            OvertimeHours = 2m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 63,
                            CheckInTime = new TimeSpan(0, 8, 30, 0, 0),
                            CheckOutTime = new TimeSpan(0, 16, 30, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 3, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 3,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 64,
                            CheckInTime = new TimeSpan(0, 8, 30, 0, 0),
                            CheckOutTime = new TimeSpan(0, 16, 30, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 4, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 3,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 65,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 5, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 3,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 66,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 6, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 3,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 67,
                            CheckInTime = new TimeSpan(0, 8, 30, 0, 0),
                            CheckOutTime = new TimeSpan(0, 16, 30, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 7, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 3,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 68,
                            CheckInTime = new TimeSpan(0, 8, 30, 0, 0),
                            CheckOutTime = new TimeSpan(0, 16, 30, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 3,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 69,
                            CheckInTime = new TimeSpan(0, 8, 30, 0, 0),
                            CheckOutTime = new TimeSpan(0, 18, 30, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 9, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 3,
                            IsActive = true,
                            OvertimeHours = 2m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 70,
                            CheckInTime = new TimeSpan(0, 8, 30, 0, 0),
                            CheckOutTime = new TimeSpan(0, 16, 30, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 10, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 3,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 71,
                            CheckInTime = new TimeSpan(0, 8, 30, 0, 0),
                            CheckOutTime = new TimeSpan(0, 16, 30, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 11, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 3,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 72,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 12, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 3,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 73,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 13, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 3,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 74,
                            CheckInTime = new TimeSpan(0, 8, 30, 0, 0),
                            CheckOutTime = new TimeSpan(0, 16, 30, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 14, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 3,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 75,
                            CheckInTime = new TimeSpan(0, 8, 30, 0, 0),
                            CheckOutTime = new TimeSpan(0, 16, 30, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 15, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 3,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 76,
                            CheckInTime = new TimeSpan(0, 8, 30, 0, 0),
                            CheckOutTime = new TimeSpan(0, 18, 30, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 16, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 3,
                            IsActive = true,
                            OvertimeHours = 2m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 77,
                            CheckInTime = new TimeSpan(0, 8, 30, 0, 0),
                            CheckOutTime = new TimeSpan(0, 16, 30, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 17, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 3,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 78,
                            CheckInTime = new TimeSpan(0, 8, 30, 0, 0),
                            CheckOutTime = new TimeSpan(0, 16, 30, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 18, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 3,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 79,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 19, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 3,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 80,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 20, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 3,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 81,
                            CheckInTime = new TimeSpan(0, 8, 30, 0, 0),
                            CheckOutTime = new TimeSpan(0, 16, 30, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 21, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 3,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 82,
                            CheckInTime = new TimeSpan(0, 8, 30, 0, 0),
                            CheckOutTime = new TimeSpan(0, 16, 30, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 22, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 3,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 83,
                            CheckInTime = new TimeSpan(0, 8, 30, 0, 0),
                            CheckOutTime = new TimeSpan(0, 18, 30, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 23, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 3,
                            IsActive = true,
                            OvertimeHours = 2m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 84,
                            CheckInTime = new TimeSpan(0, 8, 30, 0, 0),
                            CheckOutTime = new TimeSpan(0, 16, 30, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 24, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 3,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 85,
                            CheckInTime = new TimeSpan(0, 8, 30, 0, 0),
                            CheckOutTime = new TimeSpan(0, 16, 30, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 25, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 3,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 86,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 26, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 3,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 87,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 27, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 3,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 88,
                            CheckInTime = new TimeSpan(0, 8, 30, 0, 0),
                            CheckOutTime = new TimeSpan(0, 16, 30, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 28, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 3,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 89,
                            CheckInTime = new TimeSpan(0, 8, 30, 0, 0),
                            CheckOutTime = new TimeSpan(0, 16, 30, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 29, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 3,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 90,
                            CheckInTime = new TimeSpan(0, 8, 30, 0, 0),
                            CheckOutTime = new TimeSpan(0, 16, 30, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 30, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 3,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 91,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 4,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 92,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 2, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 4,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 93,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 3, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 4,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 94,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 4, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 4,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 95,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 5, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 4,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 96,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 6, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 4,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 97,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 7, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 4,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 2,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 98,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 4,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 99,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 9, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 4,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 100,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 10, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 4,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 101,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 11, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 4,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 102,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 12, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 4,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 103,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 13, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 4,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 104,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 14, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 4,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 105,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 15, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 4,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 106,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 16, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 4,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 107,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 17, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 4,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 108,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 18, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 4,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 2,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 109,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 19, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 4,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 110,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 20, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 4,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 111,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 21, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 4,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 112,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 22, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 4,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 113,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 23, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 4,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 114,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 24, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 4,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 115,
                            CheckInTime = new TimeSpan(0, 10, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 25, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 1m,
                            EmployeeId = 4,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 116,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 26, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 4,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 117,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 27, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 4,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 118,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 28, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 4,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 119,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 29, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 4,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 120,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 30, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 4,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 121,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 5,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 122,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 2, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 5,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 123,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 3, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 5,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 124,
                            CheckInTime = new TimeSpan(0, 10, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 19, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 4, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 1m,
                            EmployeeId = 5,
                            IsActive = true,
                            OvertimeHours = 2m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 125,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 5, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 5,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 126,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 6, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 5,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 127,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 7, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 5,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 128,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 5,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 129,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 9, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 5,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 130,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 10, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 5,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 131,
                            CheckInTime = new TimeSpan(0, 10, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 19, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 11, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 1m,
                            EmployeeId = 5,
                            IsActive = true,
                            OvertimeHours = 2m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 132,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 12, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 5,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 133,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 13, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 5,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 134,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 14, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 5,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 135,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 15, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 5,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 136,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 16, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 5,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 137,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 17, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 5,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 138,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 18, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 5,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 139,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 19, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 5,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 140,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 20, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 5,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 141,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 21, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 5,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 142,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 22, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 5,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 143,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 23, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 5,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 144,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 24, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 5,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 145,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 25, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 5,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 146,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 26, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 5,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 147,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 27, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 5,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 148,
                            CheckInTime = new TimeSpan(0, 10, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 28, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 1m,
                            EmployeeId = 5,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 149,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 29, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 5,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 150,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 30, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 5,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 151,
                            CheckInTime = new TimeSpan(0, 10, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 18, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 6,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 152,
                            CheckInTime = new TimeSpan(0, 10, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 18, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 2, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 6,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 153,
                            CheckInTime = new TimeSpan(0, 10, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 18, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 3, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 6,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 154,
                            CheckInTime = new TimeSpan(0, 10, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 18, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 4, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 6,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 155,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 5, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 6,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 156,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 6, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 6,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 157,
                            CheckInTime = new TimeSpan(0, 10, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 18, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 7, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 6,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 158,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 6,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 2,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 159,
                            CheckInTime = new TimeSpan(0, 10, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 18, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 9, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 6,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 160,
                            CheckInTime = new TimeSpan(0, 10, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 18, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 10, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 6,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 161,
                            CheckInTime = new TimeSpan(0, 10, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 18, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 11, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 6,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 162,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 12, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 6,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 163,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 13, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 6,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 164,
                            CheckInTime = new TimeSpan(0, 10, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 18, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 14, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 6,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 165,
                            CheckInTime = new TimeSpan(0, 10, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 20, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 15, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 6,
                            IsActive = true,
                            OvertimeHours = 2m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 166,
                            CheckInTime = new TimeSpan(0, 10, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 18, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 16, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 6,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 167,
                            CheckInTime = new TimeSpan(0, 10, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 18, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 17, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 6,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 168,
                            CheckInTime = new TimeSpan(0, 10, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 18, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 18, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 6,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 169,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 19, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 6,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 170,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 20, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 6,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 171,
                            CheckInTime = new TimeSpan(0, 10, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 18, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 21, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 6,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 172,
                            CheckInTime = new TimeSpan(0, 10, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 18, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 22, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 6,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 173,
                            CheckInTime = new TimeSpan(0, 10, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 18, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 23, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 6,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 174,
                            CheckInTime = new TimeSpan(0, 10, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 18, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 24, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 6,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 175,
                            CheckInTime = new TimeSpan(0, 10, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 18, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 25, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 6,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 176,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 26, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 6,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 177,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 27, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 6,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 178,
                            CheckInTime = new TimeSpan(0, 10, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 18, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 28, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 6,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 179,
                            CheckInTime = new TimeSpan(0, 11, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 18, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 29, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 1m,
                            EmployeeId = 6,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 180,
                            CheckInTime = new TimeSpan(0, 10, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 18, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 30, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 6,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 181,
                            CheckInTime = new TimeSpan(0, 8, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 18, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 7,
                            IsActive = true,
                            OvertimeHours = 2m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 182,
                            CheckInTime = new TimeSpan(0, 8, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 16, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 2, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 7,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 183,
                            CheckInTime = new TimeSpan(0, 8, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 16, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 3, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 7,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 184,
                            CheckInTime = new TimeSpan(0, 8, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 16, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 4, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 7,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 185,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 5, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 7,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 186,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 6, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 7,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 187,
                            CheckInTime = new TimeSpan(0, 8, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 16, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 7, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 7,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 188,
                            CheckInTime = new TimeSpan(0, 8, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 16, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 7,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 189,
                            CheckInTime = new TimeSpan(0, 8, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 16, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 9, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 7,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 190,
                            CheckInTime = new TimeSpan(0, 8, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 16, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 10, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 7,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 191,
                            CheckInTime = new TimeSpan(0, 8, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 16, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 11, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 7,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 192,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 12, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 7,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 193,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 13, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 7,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 194,
                            CheckInTime = new TimeSpan(0, 8, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 16, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 14, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 7,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 195,
                            CheckInTime = new TimeSpan(0, 8, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 16, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 15, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 7,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 196,
                            CheckInTime = new TimeSpan(0, 8, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 16, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 16, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 7,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 197,
                            CheckInTime = new TimeSpan(0, 8, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 16, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 17, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 7,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 198,
                            CheckInTime = new TimeSpan(0, 8, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 16, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 18, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 7,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 199,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 19, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 7,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 200,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 20, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 7,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 201,
                            CheckInTime = new TimeSpan(0, 8, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 16, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 21, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 7,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 202,
                            CheckInTime = new TimeSpan(0, 8, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 16, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 22, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 7,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 203,
                            CheckInTime = new TimeSpan(0, 8, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 16, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 23, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 7,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 204,
                            CheckInTime = new TimeSpan(0, 8, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 16, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 24, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 7,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 205,
                            CheckInTime = new TimeSpan(0, 8, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 16, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 25, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 7,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 206,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 26, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 7,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 207,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 27, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 7,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 208,
                            CheckInTime = new TimeSpan(0, 8, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 16, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 28, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 7,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 209,
                            CheckInTime = new TimeSpan(0, 8, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 16, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 29, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 7,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 210,
                            CheckInTime = new TimeSpan(0, 8, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 18, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 30, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 7,
                            IsActive = true,
                            OvertimeHours = 2m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 211,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 8,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 212,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 2, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 8,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 213,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 3, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 8,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 214,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 4, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 8,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 215,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 5, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 8,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 216,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 6, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 8,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 217,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 7, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 8,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 218,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 8,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 219,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 9, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 8,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 220,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 10, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 8,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 221,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 11, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 8,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 222,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 12, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 8,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 223,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 13, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 8,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 224,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 14, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 8,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 225,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 15, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 8,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 226,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 16, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 8,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 227,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 17, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 8,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 228,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 18, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 8,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 229,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 19, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 8,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 230,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 20, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 8,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 231,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 21, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 8,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 232,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 22, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 8,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 233,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 23, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 8,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 234,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 24, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 8,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 235,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 25, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 8,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 236,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 26, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 8,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 237,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 27, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 8,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 4,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 238,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 28, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 8,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 239,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 29, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 8,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 240,
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 11, 30, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            DeductionHours = 0m,
                            EmployeeId = 8,
                            IsActive = true,
                            OvertimeHours = 0m,
                            Status = 1,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        });
                });

            modelBuilder.Entity("HR.Domain.Entities.Employee", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Address")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)");

                    b.Property<decimal>("BasicSalary")
                        .HasColumnType("decimal(18,2)");

                    b.Property<DateTime>("BirthDate")
                        .HasColumnType("datetime2");

                    b.Property<TimeSpan>("CheckInTime")
                        .HasColumnType("time");

                    b.Property<TimeSpan>("CheckOutTime")
                        .HasColumnType("time");

                    b.Property<DateTime>("ContractDate")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime2")
                        .HasDefaultValueSql("GETUTCDATE()");

                    b.Property<string>("FullName")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("Gender")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<bool>("IsActive")
                        .HasColumnType("bit");

                    b.Property<string>("NationalId")
                        .IsRequired()
                        .HasMaxLength(14)
                        .HasColumnType("nvarchar(14)");

                    b.Property<string>("Nationality")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("Phone")
                        .IsRequired()
                        .HasMaxLength(11)
                        .HasColumnType("nvarchar(11)");

                    b.Property<DateTime>("UpdatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime2")
                        .HasDefaultValueSql("GETUTCDATE()");

                    b.HasKey("Id");

                    b.ToTable("Employees");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Address = "Cairo",
                            BasicSalary = 8000m,
                            BirthDate = new DateTime(1990, 5, 15, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            ContractDate = new DateTime(2015, 3, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            FullName = "Mohamed Ahmed Ismail",
                            Gender = "Male",
                            IsActive = true,
                            NationalId = "29005151234567",
                            Nationality = "Egyptian",
                            Phone = "01012345678",
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 2,
                            Address = "اGiza, Doki",
                            BasicSalary = 6500m,
                            BirthDate = new DateTime(1993, 8, 22, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            ContractDate = new DateTime(2018, 6, 15, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            FullName = "sara Mahmoud",
                            Gender = "Female",
                            IsActive = true,
                            NationalId = "29308221234568",
                            Nationality = "Egyptian",
                            Phone = "01123456789",
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 3,
                            Address = "Alexandria",
                            BasicSalary = 12000m,
                            BirthDate = new DateTime(1988, 1, 10, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            CheckInTime = new TimeSpan(0, 8, 30, 0, 0),
                            CheckOutTime = new TimeSpan(0, 16, 30, 0, 0),
                            ContractDate = new DateTime(2012, 9, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            FullName = "Ahmed Khaled Hassan",
                            Gender = "Male",
                            IsActive = true,
                            NationalId = "28801101234569",
                            Nationality = "Egyptian",
                            Phone = "01234567890",
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 4,
                            Address = "Cairo",
                            BasicSalary = 5000m,
                            BirthDate = new DateTime(1995, 3, 30, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            ContractDate = new DateTime(2020, 1, 5, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            FullName = "Menna Ibrahim",
                            Gender = "Female",
                            IsActive = true,
                            NationalId = "29503301234560",
                            Nationality = "Egyptian",
                            Phone = "01098765432",
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 5,
                            Address = "October",
                            BasicSalary = 9500m,
                            BirthDate = new DateTime(1991, 11, 5, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            ContractDate = new DateTime(2016, 4, 10, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            FullName = "Omar Yousef",
                            Gender = "Male",
                            IsActive = true,
                            NationalId = "29111051234561",
                            Nationality = "Egyptian",
                            Phone = "01511223344",
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 6,
                            Address = "Maadi",
                            BasicSalary = 4500m,
                            BirthDate = new DateTime(1997, 6, 18, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            CheckInTime = new TimeSpan(0, 10, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 18, 0, 0, 0),
                            ContractDate = new DateTime(2021, 2, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            FullName = "Nourahan Ali",
                            Gender = "Female",
                            IsActive = true,
                            NationalId = "29706181234562",
                            Nationality = "Egyptian",
                            Phone = "01677889900",
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 7,
                            Address = "Cairo",
                            BasicSalary = 15000m,
                            BirthDate = new DateTime(1985, 9, 25, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            CheckInTime = new TimeSpan(0, 8, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 16, 0, 0, 0),
                            ContractDate = new DateTime(2010, 7, 20, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            FullName = "Kareem ali",
                            Gender = "Male",
                            IsActive = true,
                            NationalId = "28509251234563",
                            Nationality = "Egyptian",
                            Phone = "01022334455",
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 8,
                            Address = "Giza",
                            BasicSalary = 7000m,
                            BirthDate = new DateTime(1992, 4, 12, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            CheckInTime = new TimeSpan(0, 9, 0, 0, 0),
                            CheckOutTime = new TimeSpan(0, 17, 0, 0, 0),
                            ContractDate = new DateTime(2019, 11, 3, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            FullName = "Reem Adel",
                            Gender = "Female",
                            IsActive = true,
                            NationalId = "29204121234564",
                            Nationality = "Egyptian",
                            Phone = "01155667788",
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        });
                });

            modelBuilder.Entity("HR.Domain.Entities.GeneralsSettings", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<decimal>("DeductionHourValue")
                        .HasPrecision(10, 2)
                        .HasColumnType("decimal(10,2)");

                    b.Property<decimal>("OvertimeHourValue")
                        .HasPrecision(10, 2)
                        .HasColumnType("decimal(10,2)");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.Property<string>("WeeklyHolidayDay1")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("WeeklyHolidayDay2")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.ToTable("GeneralSettings");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            DeductionHourValue = 2m,
                            OvertimeHourValue = 2m,
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            WeeklyHolidayDay1 = "الجمعة",
                            WeeklyHolidayDay2 = "السبت"
                        });
                });

            modelBuilder.Entity("HR.Domain.Entities.Officialholiday", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("Date")
                        .HasColumnType("datetime2");

                    b.Property<bool>("IsActive")
                        .HasColumnType("bit");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.HasKey("Id");

                    b.ToTable("Officialholidays");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IsActive = true,
                            Name = "رأس السنة الميلادية",
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 2,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 1, 25, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IsActive = true,
                            Name = "عيد الشرطة",
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 3,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 1, 25, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IsActive = true,
                            Name = "ثورة 25 يناير",
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 4,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 5, 3, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IsActive = true,
                            Name = "شم النسيم",
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 5,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 4, 25, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IsActive = true,
                            Name = "عيد تحرير سيناء",
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 6,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 10, 6, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IsActive = true,
                            Name = "نصر 6 أكتوبر",
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 7,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 7, 23, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IsActive = true,
                            Name = "ثورة 23 يوليو",
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 8,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Date = new DateTime(2021, 10, 21, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IsActive = true,
                            Name = "المولد النبوى الشريف",
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        });
                });

            modelBuilder.Entity("HR.Domain.Identity.AppUser", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("nvarchar(150)");

                    b.Property<string>("FullName")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<bool>("IsActive")
                        .HasColumnType("bit");

                    b.Property<string>("PasswordHash")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.Property<int>("UserGroupId")
                        .HasColumnType("int");

                    b.Property<string>("Username")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.HasKey("Id");

                    b.HasIndex("Email")
                        .IsUnique();

                    b.HasIndex("UserGroupId");

                    b.HasIndex("Username")
                        .IsUnique();

                    b.ToTable("Users");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Email = "admin@pioneers-solutions.com",
                            FullName = "System Admin",
                            IsActive = true,
                            PasswordHash = "$2a$11$c84awI5hkIxiejS8ehyYoO4wifxC3QALC4vtmm7PEbz1IuZ8LdSay",
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            UserGroupId = 1,
                            Username = "admin"
                        },
                        new
                        {
                            Id = 2,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Email = "hr@pioneers-solutions.com",
                            FullName = "HR Manager",
                            IsActive = true,
                            PasswordHash = "$2a$11$lxmRaOydGZpP2NQj/xwyUeU6hRcmBh9491u8jqXFFIV0get5lx92u",
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            UserGroupId = 2,
                            Username = "hrmanager"
                        });
                });

            modelBuilder.Entity("HR.Domain.Identity.GroupPermission", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int>("Action")
                        .HasColumnType("int");

                    b.Property<int>("Screen")
                        .HasColumnType("int");

                    b.Property<int>("UserGroupId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("UserGroupId", "Screen", "Action")
                        .IsUnique();

                    b.ToTable("GroupPermissions");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Action = 1,
                            Screen = 1,
                            UserGroupId = 1
                        },
                        new
                        {
                            Id = 2,
                            Action = 2,
                            Screen = 1,
                            UserGroupId = 1
                        },
                        new
                        {
                            Id = 3,
                            Action = 3,
                            Screen = 1,
                            UserGroupId = 1
                        },
                        new
                        {
                            Id = 4,
                            Action = 4,
                            Screen = 1,
                            UserGroupId = 1
                        },
                        new
                        {
                            Id = 5,
                            Action = 1,
                            Screen = 2,
                            UserGroupId = 1
                        },
                        new
                        {
                            Id = 6,
                            Action = 2,
                            Screen = 2,
                            UserGroupId = 1
                        },
                        new
                        {
                            Id = 7,
                            Action = 3,
                            Screen = 2,
                            UserGroupId = 1
                        },
                        new
                        {
                            Id = 8,
                            Action = 4,
                            Screen = 2,
                            UserGroupId = 1
                        },
                        new
                        {
                            Id = 9,
                            Action = 1,
                            Screen = 3,
                            UserGroupId = 1
                        },
                        new
                        {
                            Id = 10,
                            Action = 2,
                            Screen = 3,
                            UserGroupId = 1
                        },
                        new
                        {
                            Id = 11,
                            Action = 3,
                            Screen = 3,
                            UserGroupId = 1
                        },
                        new
                        {
                            Id = 12,
                            Action = 4,
                            Screen = 3,
                            UserGroupId = 1
                        },
                        new
                        {
                            Id = 13,
                            Action = 1,
                            Screen = 4,
                            UserGroupId = 1
                        },
                        new
                        {
                            Id = 14,
                            Action = 2,
                            Screen = 4,
                            UserGroupId = 1
                        },
                        new
                        {
                            Id = 15,
                            Action = 3,
                            Screen = 4,
                            UserGroupId = 1
                        },
                        new
                        {
                            Id = 16,
                            Action = 4,
                            Screen = 4,
                            UserGroupId = 1
                        },
                        new
                        {
                            Id = 17,
                            Action = 1,
                            Screen = 5,
                            UserGroupId = 1
                        },
                        new
                        {
                            Id = 18,
                            Action = 2,
                            Screen = 5,
                            UserGroupId = 1
                        },
                        new
                        {
                            Id = 19,
                            Action = 3,
                            Screen = 5,
                            UserGroupId = 1
                        },
                        new
                        {
                            Id = 20,
                            Action = 4,
                            Screen = 5,
                            UserGroupId = 1
                        },
                        new
                        {
                            Id = 21,
                            Action = 1,
                            Screen = 6,
                            UserGroupId = 1
                        },
                        new
                        {
                            Id = 22,
                            Action = 2,
                            Screen = 6,
                            UserGroupId = 1
                        },
                        new
                        {
                            Id = 23,
                            Action = 3,
                            Screen = 6,
                            UserGroupId = 1
                        },
                        new
                        {
                            Id = 24,
                            Action = 4,
                            Screen = 6,
                            UserGroupId = 1
                        },
                        new
                        {
                            Id = 25,
                            Action = 1,
                            Screen = 7,
                            UserGroupId = 1
                        },
                        new
                        {
                            Id = 26,
                            Action = 2,
                            Screen = 7,
                            UserGroupId = 1
                        },
                        new
                        {
                            Id = 27,
                            Action = 3,
                            Screen = 7,
                            UserGroupId = 1
                        },
                        new
                        {
                            Id = 28,
                            Action = 4,
                            Screen = 7,
                            UserGroupId = 1
                        },
                        new
                        {
                            Id = 29,
                            Action = 1,
                            Screen = 1,
                            UserGroupId = 2
                        },
                        new
                        {
                            Id = 30,
                            Action = 2,
                            Screen = 1,
                            UserGroupId = 2
                        },
                        new
                        {
                            Id = 31,
                            Action = 3,
                            Screen = 1,
                            UserGroupId = 2
                        },
                        new
                        {
                            Id = 32,
                            Action = 4,
                            Screen = 1,
                            UserGroupId = 2
                        },
                        new
                        {
                            Id = 33,
                            Action = 1,
                            Screen = 2,
                            UserGroupId = 2
                        },
                        new
                        {
                            Id = 34,
                            Action = 2,
                            Screen = 2,
                            UserGroupId = 2
                        },
                        new
                        {
                            Id = 35,
                            Action = 3,
                            Screen = 2,
                            UserGroupId = 2
                        },
                        new
                        {
                            Id = 36,
                            Action = 4,
                            Screen = 2,
                            UserGroupId = 2
                        },
                        new
                        {
                            Id = 37,
                            Action = 1,
                            Screen = 3,
                            UserGroupId = 2
                        },
                        new
                        {
                            Id = 38,
                            Action = 2,
                            Screen = 3,
                            UserGroupId = 2
                        },
                        new
                        {
                            Id = 39,
                            Action = 3,
                            Screen = 3,
                            UserGroupId = 2
                        },
                        new
                        {
                            Id = 40,
                            Action = 4,
                            Screen = 3,
                            UserGroupId = 2
                        },
                        new
                        {
                            Id = 41,
                            Action = 1,
                            Screen = 4,
                            UserGroupId = 2
                        },
                        new
                        {
                            Id = 42,
                            Action = 2,
                            Screen = 4,
                            UserGroupId = 2
                        },
                        new
                        {
                            Id = 43,
                            Action = 3,
                            Screen = 4,
                            UserGroupId = 2
                        },
                        new
                        {
                            Id = 44,
                            Action = 4,
                            Screen = 4,
                            UserGroupId = 2
                        },
                        new
                        {
                            Id = 45,
                            Action = 1,
                            Screen = 5,
                            UserGroupId = 2
                        },
                        new
                        {
                            Id = 46,
                            Action = 2,
                            Screen = 5,
                            UserGroupId = 2
                        },
                        new
                        {
                            Id = 47,
                            Action = 3,
                            Screen = 5,
                            UserGroupId = 2
                        },
                        new
                        {
                            Id = 48,
                            Action = 4,
                            Screen = 5,
                            UserGroupId = 2
                        });
                });

            modelBuilder.Entity("HR.Domain.Identity.UserGroup", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<bool>("IsActive")
                        .HasColumnType("bit");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.HasKey("Id");

                    b.HasIndex("Name")
                        .IsUnique();

                    b.ToTable("UserGroups");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IsActive = true,
                            Name = "Administrators",
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 2,
                            CreatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IsActive = true,
                            Name = "HR Staff",
                            UpdatedAt = new DateTime(2021, 11, 8, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        });
                });

            modelBuilder.Entity("HR.Domain.Entities.Attendance", b =>
                {
                    b.HasOne("HR.Domain.Entities.Employee", "Employee")
                        .WithMany("Attendances")
                        .HasForeignKey("EmployeeId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Employee");
                });

            modelBuilder.Entity("HR.Domain.Identity.AppUser", b =>
                {
                    b.HasOne("HR.Domain.Identity.UserGroup", "UserGroup")
                        .WithMany("Users")
                        .HasForeignKey("UserGroupId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("UserGroup");
                });

            modelBuilder.Entity("HR.Domain.Identity.GroupPermission", b =>
                {
                    b.HasOne("HR.Domain.Identity.UserGroup", "UserGroup")
                        .WithMany("Permissions")
                        .HasForeignKey("UserGroupId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("UserGroup");
                });

            modelBuilder.Entity("HR.Domain.Entities.Employee", b =>
                {
                    b.Navigation("Attendances");
                });

            modelBuilder.Entity("HR.Domain.Identity.UserGroup", b =>
                {
                    b.Navigation("Permissions");

                    b.Navigation("Users");
                });
#pragma warning restore 612, 618
        }
    }
}
